 ...
 *
 * Functions for computing word-level diffs between documents
 * with character position tracking for accurate document modifications.
 */

import { diffWords, type Change as DiffChange } from "diff";
...
/**
 * Compute word-level changes between two texts with character position tracking.
 *
 * This function performs a word-level diff and tracks:
...
export function getDeletionSearchContext(
  ...
  if (!change.contextBefore || change.contextBefore.length < MIN_CONTEXT_LENGTH) {
    return undefined;
  }
